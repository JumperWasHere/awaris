...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      var heirId = await this.createHeir();
      if (heirId) {
        await this.storeDocument(heirId, this.mykad.front, "mykad-front");
        await this.storeDocument(heirId, this.mykad.back, "mykad-back");
        this.$router.push({
          name: "trust.part-b",
          query: {
    ...
    async createHeir() {
      console.log("HIBAH: ", this.trustId);
