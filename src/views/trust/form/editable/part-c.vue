...
export default {
  ...
  methods: {
    getOrgnizations() {
      ApiService.get(
        `organization?allow_hibah=1&except_short_name=ONE CENT`
      ).then((response) => {
    ...
    getBranchesByOrganization() {
      ApiService.get(
        `organization/${this.organizationId}/branch?status=1`
      ).then((response) => {
    ...
    getOrganizationUsers(branchid) {
      let query;
      if (!branchid) {
        query = `organization/${this.organizationId}/user?status=1`;
      } else {
        query = `organization/${this.organizationId}/user?branch_id=${branchid}&status=1`;
      }
    ...
    async getPricings() {
      ...
      return await ApiService.get(
        `/organization/${this.organizationId}/pricing?product=Hibah&status=1`
      )
        .then((response) => {
    ...
    async updateHibah() {
      ...
      if (this.userRole == "Staff") {
        ...
        hibah.branch_id = this.headquater;
      } else if (["Admin", "Agent"].includes(this.userRole)) {
        hibah.branch_id = this.userAccess.accessible.id;
