...
export default {
  ...
  watch: {
    AddReceiver: {
      handler: function () {
        console.log("execute");
        if (this.hibah && ["1", "2"].includes(this.hibah.hibah_type)) {
  ...
  methods: {
    getReceivers() {
      ApiService.get(`hibah/${this.hibahId}/receiver`).then((response) => {
        ...
        <div class="row">
          <div class="form-group col-md-6">
            <label for="receiver_id">
              Penerima
              <img
                content="Jika penerima tidak tersenarai, sila daftarkan terlebih dahulu di Bahagian F"
                v-tippy
                height="15"
                class="mb-1"
