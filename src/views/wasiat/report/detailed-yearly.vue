...
export default {
  ...
  filters: {
    ...
    toHumanDate: function (value) {
      return moment(value, "YYYY").format("YYYY");
    },
  },
  ...
  methods: {
    getWasiats() {
      ApiService.get(
        `/wasiat/detailed-report?format=html&date=${this.currentYear}&organization_id=${this.organizationId}&branch_id=${this.branchId}`
      ).then((response) => {
    ...
    downloadExcel() {
      window.open(
        process.env.VUE_APP_API_URL +
          `/wasiat/detailed-report?format=excel&date=${this.currentYear}&organization_id=${this.organizationId}&branch_id=${this.branchId}`,
        "_blank"
      );
    },
    yearChange() {
...
<template>
  <div>
    ...
    <section class="section">
      ...
      >
        <div class="card-body">
          <h5 class="text-center">AWARIS DETAILED YEARLY REPORT</h5>
